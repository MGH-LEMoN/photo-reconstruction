...
for id = 1:length(subjects)
    for skip_id = 1:length(skips)
        ...
        target_intensities = ['/space/calico/1/users/Harsha/photo-reconstruction/results/recons/' subjects{id} '/ref_mask_skip_' skips{skip_id} '/photo_recon.mgz'];
        output_segmentation = [subjects{id} '_seg_output.mgz'];
        output_QC_prefix = [subjects{id} '_seg_output_QC'];
        try
            propagate_manual_segs_slices_elastix_smart(reference_intensities, reference_segmentation, target_intensities, output_segmentation, output_QC_prefix, skips{skip_id})
        catch
            disp("FAILED")
            disp(subjects{id})
            disp(skips{skip_id})
