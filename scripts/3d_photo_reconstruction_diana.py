...
parser.add_argument("--ref_soft_mask",
                    ...
                    default=None)
parser.add_argument("--ref_soft_image",
                    type=str,
                    help="Reference soft image (atlas)",
                    default=None)
...
n_refs = ((options.ref_mask is not None) +
          ...
          (options.ref_soft_image is not None))
if n_refs != 1:
    raise Exception(
        "You should provide 1 and only 1 reference: surface or soft/hard mask/image"
    )
...
if options.ref_mask is not None:
    ...
    ref_type = "image"
elif options.ref_soft_image is not None:
    input_reference = options.ref_soft_image
    ref_type = "soft_image"
else:
...
if ref_type != "surface":
    ...
    if ref_type == "mask" or ref_type == "image" or ref_type == "soft_image":
        if ref_type == "soft_image":
            REF = (REF > (0.05*np.max(REF))).astype("float")
        else:
...
for mode_idx in range(n_modes):
    ...
    if mode_idx == 0:
        ...
        if ref_type == "image" or ref_type == "soft_image":
        ...
        if ref_type == "image" or ref_type == "soft_image":
            ...
            if DL_synthesis_model is None:
                ...
                tempdir = (output_directory + '\\temp\\')
                if os.path.isdir(tempdir):
                    print('del  ' + tempdir + '\\*.*')
                    os.system('del /q ' + tempdir + '\\*.*')
                else:
                    os.mkdir(tempdir)
                tempdir2 = (output_directory + '\\temp2\\')
                if os.path.isdir(tempdir2):
                    print('del  ' + tempdir2 + '\\*.*')
                    os.system('del  /q ' + tempdir2 + '\\*.*')
                else:
                    os.mkdir(tempdir2)
                for z in range(Nslices):
                    cv2.imwrite(tempdir + str(z) + '.png', np.mean(Is[-1][:, :, z], axis=-1).astype('B'))
                scriptdir = 'C:/Users/diana/Downloads/softwares/SynthSR-main'
                os.system(scriptdir + '/run_synthesis.bat ' + tempdir + ' ' + tempdir2 + ' ' + DL_synthesis_model)
                print(scriptdir + '/run_synthesis.bat ' + tempdir + ' ' + tempdir2 + ' ' + DL_synthesis_model)
                print(tempdir2)
                for z in range(Nslices):
...
if False: #DL_synthesis_model is not None:
    os.system('del /q ' + tempdir + '\\*.*')
    os.system('del /q ' + tempdir2 + '\\*.*')
    os.system('rm -rf ' + tempdir )
    os.system('rm -rf ' + tempdir2 )
