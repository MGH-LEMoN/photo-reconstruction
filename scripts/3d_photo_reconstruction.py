...
parser.add_argument('--input_photo_dir', type=str, nargs='*', help='Directory with input photos (required)', required=True)
parser.add_argument('--input_segmentation_dir', type=str, nargs='*', help='Directory with input slab masks / segmentations (required)', required=True)

parser.add_argument('--ref_mask', type=str, help='Reference binary mask', default=None)
parser.add_argument('--ref_surface', type=str, help='Reference surface file', default=None)
parser.add_argument('--ref_image', type=str, help='Reference image file', default=None)
parser.add_argument('--ref_soft_mask', type=str, help='Reference soft mask', default=None)

parser.add_argument('--mesh_autoalign_target', type=str, help='Probabilistic atlas to globally initialize mesh rotation', default=None)
parser.add_argument('--mesh_manually_oriented', dest='mesh_manually_oriented', action='store_true', \
...
parser.add_argument('--allow_z_stretch', dest='allow_z_stretch', action='store_true',\
...
parser.add_argument('--slice_thickness', type=float, help='Slice thickness in mm', required=True)
parser.add_argument('--photo_resolution', type=float, help='Resolution of the photos in mm', required=True)
parser.add_argument('--n_cp_nonlin', type=int, help='number of control points for within slice nonlinear deformation of ' +\
                                                    ' the photos, along largest dimension of image. You should probably ' +\
                                                    ' *never* use this with soft references', default=0)

parser.add_argument('--stiffness_nonlin', type=float, help='stiffness of the nonlinear deformation', default=0.33333333333333)

parser.add_argument('--output_directory', type=str, help='Output directory with reconstructed photo volume and reference', required=True)
...
if True:
    ...
    # Hard
    command_line =  ' --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB ' + \
                    ' --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB ' + \
                    ' --slice_thickness 4.0 ' + \
                    ' --photo_resolution 0.1 ' + \
                    ' --photos_of_posterior_side ' + \
                    ' --ref_mask /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_mask.nii.gz' + \
                    ' --allow_z_stretch ' + \
                    ' --output_directory  /tmp/photo_recon.hard/ ' + \
                    '   '  + ' --gpu 0'  # + ' --n_cp_nonlin  6 --stiffness_nonlin 0.4'
    #
    # python scripts/3d_photo_reconstruction.py --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB  --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB --slice_thickness 4.0  --photo_resolution 0.1 --photos_of_posterior_side  --ref_mask /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_mask.nii.gz --allow_z_stretch  --output_directory  /tmp/photo_recon.hard/  --gpu 0
    ...
    #                 ' --ref_mask /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_mask.nii.gz' + \
    #                 ' --allow_z_stretch ' + \
    #                 ' --output_directory  /tmp/photo_recon.hard.nonlinear/ ' + \
    #                 '   '  + ' --gpu 0'  + ' --n_cp_nonlin  6 --stiffness_nonlin 0.4'
    #
    # python scripts/3d_photo_reconstruction.py --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB  --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB --slice_thickness 4.0  --photo_resolution 0.1 --photos_of_posterior_side  --ref_mask /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_mask.nii.gz --allow_z_stretch  --output_directory  /tmp/photo_recon.hard.nonlinear/  --gpu 0  --n_cp_nonlin  10 --stiffness_nonlin 0.4
    ...
    #                ' --output_directory  /tmp/photo_recon.soft/ ' + \
    #                '   ' + ' --gpu 0'

    # python scripts/3d_photo_reconstruction.py  --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB  --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB --slice_thickness 4.0  --photo_resolution 0.1 --photos_of_posterior_side   --ref_soft_mask /autofs/cluster/vive/UW_photo_recon/prob_atlases/onlyCerebrum.nii.gz --output_directory  /tmp/photo_recon.soft/   --gpu 0
    ...
    #                 ' --photo_resolution 0.1 ' + \
    #                 ' --photos_of_posterior_side ' + \
    #                 ' --ref_image  /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_masked.nii.gz ' + \
    #                 ' --allow_z_stretch ' + \
    #                 ' --output_directory  /tmp/photo_recon.image/ ' + \
    #                 '   ' + ' --gpu 0 '
    #
    # python scripts/3d_photo_reconstruction.py --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB   --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB   --slice_thickness 4.0 --photo_resolution 0.1  --photos_of_posterior_side --ref_image  /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_masked.nii.gz  --allow_z_stretch  --output_directory  /tmp/photo_recon.image/  --gpu 0
    ...
    #                 ' --ref_image  /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_masked.nii.gz ' + \
    #                 ' --allow_z_stretch ' + \
    #                 ' --output_directory  /tmp/photo_recon.image.nonlinear/ ' + \
    #                 '   ' + ' --gpu 0 ' + ' --n_cp_nonlin  10 --stiffness_nonlin 0.4 '
    #
    # python scripts/3d_photo_reconstruction.py --input_photo_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB   --input_segmentation_dir /autofs/cluster/vive/UW_photo_recon/Photo_data/18-1132/18-1132 MATLAB   --slice_thickness 4.0 --photo_resolution 0.1  --photos_of_posterior_side --ref_image  /autofs/cluster/vive/UW_photo_recon/FLAIR_Scan_Data/NP18_1132.rotated_masked.nii.gz  --allow_z_stretch  --output_directory  /tmp/photo_recon.image.nonlinear/  --gpu 0 --n_cp_nonlin  10 --stiffness_nonlin 0.4

    # Mesh
    # command_line =  ' --input_photo_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/deformed/ ' + \
    #                 ' --input_segmentation_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/index_segs ' + \
    #                 ' --slice_thickness 8.0 ' + \
    #                 ' --photo_resolution 0.1 ' + \
    #                 ' --photos_of_posterior_side ' + \
    #                 ' --ref_surface  /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/mesh/lh_fix_48_HDR_101921.stl ' + \
    #                 ' --mesh_autoalign_target /autofs/cluster/vive/UW_photo_recon/prob_atlases/onlyCerebrum.left_hemi.smoothed.nii.gz ' + \
    #                 ' --allow_z_stretch ' + \
    #                 ' --output_directory /tmp/photo_recon.mesh/ ' + \
    #                 '   ' + ' --gpu 0 '
    #
    # python scripts/3d_photo_reconstruction.py  --input_photo_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/deformed/  --input_segmentation_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/index_segs  --slice_thickness 8.0  --photo_resolution 0.1   --photos_of_posterior_side   --ref_surface  /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/mesh/lh_fix_48_HDR_101921.stl  --mesh_autoalign_target /autofs/cluster/vive/UW_photo_recon/prob_atlases/onlyCerebrum.left_hemi.smoothed.nii.gz   --allow_z_stretch   --output_directory /tmp/photo_recon.mesh/  --gpu 0
    ...
    #                     ' --mesh_autoalign_target /autofs/cluster/vive/UW_photo_recon/prob_atlases/onlyCerebrum.left_hemi.smoothed.nii.gz ' + \
    #                     ' --allow_z_stretch ' + \
    #                     ' --output_directory /tmp/photo_recon.mesh.nonlinear/ ' + \
    #                     '   ' + ' --gpu 0 '  + ' --n_cp_nonlin  10 --stiffness_nonlin 0.4' # + ' --mesh_manually_oriented '
    #
    # python scripts/3d_photo_reconstruction.py  --input_photo_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/deformed/  --input_segmentation_dir /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/photos/index_segs  --slice_thickness 8.0  --photo_resolution 0.1   --photos_of_posterior_side   --ref_surface  /autofs/cluster/vive/MGH_photo_recon/data/20211019_2607_left/mesh/lh_fix_48_HDR_101921.stl  --mesh_autoalign_target /autofs/cluster/vive/UW_photo_recon/prob_atlases/onlyCerebrum.left_hemi.smoothed.nii.gz   --allow_z_stretch   --output_directory /tmp/photo_recon.mesh.nonlinear/  --gpu 0 --n_cp_nonlin  10 --stiffness_nonlin 0.4

    options = parser.parse_args(command_line.split())
...
if options.gpu is None:
    ...
    if torch.cuda.is_available():
        ...
        print('Tried to use GPU device ' + str(options.gpu) + ' but failed; using CPU instead')
        device = torch.device('cpu')
...
n_refs = (options.ref_mask is not None) + (options.ref_soft_mask is not None) \
         + (options.ref_surface is not None) + (options.ref_image is not None)
if n_refs != 1:
    raise Exception('You should provide 1 and only 1 reference: binary mask, soft mask, surface, or image')
...
for i in range(len(options.input_segmentation_dir)-1):
    input_segmentation_dir = input_segmentation_dir + ' ' + options.input_segmentation_dir[i+1]
...
if n_cp_nonlin >0 and n_cp_nonlin < 4:
    raise Exception('If you are using the nonlinear mode, the minimum number of control points is 5')
...
for n in np.arange(Nslices):
    Isl = cv2.resize(Iorig[n], None, fx=photo_res/RESOLUTIONS[-1], fy=photo_res/RESOLUTIONS[-1], interpolation=cv2.INTER_AREA)
    Msl = cv2.resize(Morig[n].astype('float'), None, fx=photo_res/RESOLUTIONS[-1], fy=photo_res/RESOLUTIONS[-1], interpolation=cv2.INTER_AREA)
    Isl[Msl==0] = 0
    I.append(Isl)
    M.append(Msl)
...
aff = np.array([[0, -RESOLUTIONS[-1], 0, 0], [0, 0, -slice_thickness, 0], [-RESOLUTIONS[-1], 0, 0, 0], [0, 0, 0, 1]])
if reverse_lr:
    aff[0, 1] = -aff[0, 1]
...
for s in np.arange(Nscales-2,-1,-1):
    ...
    if reverse_lr:
        aff[0, 1] = -aff[0, 1]
    aux = np.array([[RESOLUTIONS[s]/RESOLUTIONS[-1]], [RESOLUTIONS[s]/RESOLUTIONS[-1]], [1]])
...
# Surfaces require quite a bit of extra work
    ...
    original_mesh_decimated = output_directory + '/input_mesh_with_header.decimated.surf'
    ...
    reoriented_mask_vol = output_directory + '/input_mesh_with_header.filled.reoriented.mgz'
    reoriented_mesh_decimated = output_directory + '/input_mesh_with_header.decimated.reoriented.surf'
    ...
    if mesh_manually_oriented:
        if not os.path.isfile(original_mesh_decimated):
            raise Exception('Decimated mesh not found in output directory. Did you try running the code in automatic mode first? (see manual)')
        if not os.path.isfile(original_mesh_full):
            raise Exception('Full-res mesh not found in output directory. Did you try running the code in automatic mode first? (see manual)')
        if not os.path.isfile(original_mask_vol):
            raise Exception('Original mask (filled mesh) volume not found in output directory. Did you try running the code in automatic mode first? (see manual)')
        if not os.path.isfile(reoriented_mask_vol):
            raise Exception('Reoriented mask (filled mesh) volumes not found in output directory. Did you try running the code in automatic mode first? (see manual)')
        ...
        Pfull, Tfull, meta_full = nib.freesurfer.read_geometry(original_mesh_full, read_metadata=True)
        Pfull += meta_full['cras']  # ** CRUCIAL **
        meta_full['cras'][:] = 0
        Pfull_oriented = np.matmul(np.concatenate([Pfull, np.ones([Pfull.shape[0], 1])], axis=1), T.transpose())[:, :-1]
        nib.freesurfer.write_geometry(reoriented_mesh_full, Pfull_oriented, Tfull, volume_info=meta_full)

        Pdec, Tdec, meta_dec = nib.freesurfer.read_geometry(original_mesh_decimated, read_metadata=True)
        Pdec += meta_dec['cras']  # ** CRUCIAL **
        meta_dec['cras'][:] = 0
        Pdec_oriented = np.matmul(np.concatenate([Pdec, np.ones([Pdec.shape[0], 1])], axis=1), T.transpose())[:, :-1]
        nib.freesurfer.write_geometry(reoriented_mesh_decimated, Pdec_oriented, Tdec, volume_info=meta_dec)

    else:
        fs_home = os.getenv('FREESURFER_HOME')
        if fs_home is None:
            raise Exception('FREESURFER_HOME variable not found; is FreeSurfer sourced?')

        # Load reference mesh and apply header, unless already there
        if os.path.isfile(output_directory + '/input_mesh_with_header.surf'):
            print('Reference mesh with header already found in output directory; skipping computation')
        else:
            print('Loading reference mesh and applying existing header')
            a = os.system('mris_copy_header ' + input_reference + ' '  + fs_home + '/subjects/bert/surf/rh.white ' + output_directory + '/input_mesh_with_header.surf >/dev/null')
            if a > 0:
        ...
        if os.path.isfile(output_directory + '/input_mesh_with_header.filled.mgz'):
            print('Filled in mesh volume already found in output directory; skipping computation')
        else:
            print('Filling in mesh to obtain binary volume')
            a = os.system('mris_fill -r 1 ' + output_directory + '/input_mesh_with_header.surf ' + output_directory + '/temp.mgz >/dev/null')
            if a > 0:
            ...
            aff2 = aff
            aff2[:-1, -1] = aff2[:-1, -1] - np.squeeze(np.matmul(aff[:-1, :-1], pad * np.ones([3, 1])))
            my.MRIwrite(img2, aff2, output_directory + '/input_mesh_with_header.filled.mgz')
        ...
        if os.path.isfile(output_directory + '/input_mesh_with_header.decimated.surf'):
            print('Decimated mesh volume already found in output directory; skipping computation')
        else:
            print('Decimating mesh - useful for some operations that would otherwise take forever')
            a = os.system('mris_remesh  -i ' + output_directory + '/input_mesh_with_header.surf  -o ' + \
                          output_directory + '/input_mesh_with_header.decimated.surf --nvert 10000 >/dev/null')
            if a>0:
                raise Exception('mris_remesh failed')

        # OK now we are ready for automated  alignment
        if os.path.isfile(reoriented_mesh_decimated) and os.path.isfile(reoriented_mesh_full) and os.path.isfile(reoriented_mask_vol):
            print('Reoriented mesh and filled volume volume already found in output directory; skipping alignment')
        else:
            print('Aligning mesh to provided probabilistic atlas for automatic reorientation (can be a bit slow...)')
            ...
            Pdec, Tdec, meta_dec = nib.freesurfer.read_geometry(output_directory + 'input_mesh_with_header.decimated.surf', read_metadata=True)
            Pdec += meta_dec['cras']  # ** CRUCIAL **
            meta_dec['cras'][:] = 0  # We can now easily write surfaces in stl or surf
            mesh_dec = trimesh.Trimesh(Pdec, Tdec, process=False)
            T, cost = trimesh.registration.mesh_other(mesh_dec, target_pc, samples=500, scale=False, icp_first=10, icp_final=25)

            # Write registered meshes at full and decimated resolution
            Pdec_oriented = np.matmul(np.concatenate([Pdec, np.ones([Pdec.shape[0], 1])], axis=1), T.transpose())[:,:-1]
            nib.freesurfer.write_geometry(reoriented_mesh_decimated, Pdec_oriented, Tdec, volume_info=meta_dec)

            Pfull, Tfull, meta_full = nib.freesurfer.read_geometry(output_directory + 'input_mesh_with_header.surf', read_metadata=True)
            Pfull += meta_full['cras']  # ** CRUCIAL **
            meta_full['cras'][:] = 0
            Pfull_oriented = np.matmul(np.concatenate([Pfull, np.ones([Pfull.shape[0], 1])], axis=1), T.transpose())[:,:-1]
            nib.freesurfer.write_geometry(reoriented_mesh_full, Pfull_oriented, Tfull, volume_info=meta_full)
            ...
            img, aff = my.MRIread(output_directory + '/input_mesh_with_header.filled.mgz')
    ...
    Pfull, Tfull, meta_full = nib.freesurfer.read_geometry(reoriented_mesh_full, read_metadata=True)
    Pfull += meta_full['cras']  # ** CRUCIAL **
    meta_full['cras'][:] = 0

    Pdec, Tdec, meta_dec = nib.freesurfer.read_geometry(reoriented_mesh_decimated, read_metadata=True)
    Pdec += meta_dec['cras']  # ** CRUCIAL **
    meta_dec['cras'][:] = 0
...
for mode_idx in range(n_modes):
    ...
    for res in range(len(RESOLUTIONS)):
        ...
        print('Working on resolution %d of %d (%.2f mm): %d iterations ' % (res+1, len(RESOLUTIONS), RESOLUTIONS[res], STEPS[res] ))
        ...
        model = PRnets.PhotoAligner(Is[res], Ms[res], Affs[res], REFsmooth, REFaff, ref_surface,
                t_ini=t, theta_ini=theta, shear_ini=shear, scaling_ini=scaling, sz_ini=sz, allow_sz=allow_z_stretch,
                t_reference_ini=t_reference, theta_reference_ini=theta_reference, s_reference_ini=s_reference, allow_s_reference=allow_s_reference,
                ref_type=ref_type_iteration, field_ini=field, allow_nonlin=allow_nonlin,
                k_dice_mri=k_dice_mri_iteration, k_ncc_intermodality=k_ncc_intermodality_iteration, k_surface_term=k_surface_term_iteration,
                k_dice_slices=K_DICE_SLICES, k_ncc_slices=K_NCC_SLICES, k_regularizer=K_REGULARIZER,
                pad_ignore=PAD_AP, device=device, allow_scaling_and_shear=allow_scaling_and_shear, k_nonlinear=K_NONLINEAR)
        ...
        if res == (len(RESOLUTIONS)-1) and mode_idx==n_modes-1:
            ...
            model.photo_vol = torch.Tensor(Is_copy).to(device)  # TODO: I had a division by np.max(Is_copy) but got rid of it...
            model.photo_rearranged = torch.unsqueeze(model.photo_vol.permute(3, 0, 1, 2), dim=0).to(model.device)
...
if ref_type=='surface':
    ...
    Pdec_rotated = np.matmul(np.concatenate([Pdec, np.ones([Pdec.shape[0], 1])], axis=1), Rt.transpose())[:, :-1]
    nib.freesurfer.write_geometry(output_registered_reference[:-4] + 'decimated.surf', Pdec_rotated, Tdec, volume_info=meta_dec)
