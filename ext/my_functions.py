...
def downsampleMRI2d(X, aff, shape, factors, mode='image'):
    ...
    assert (shape is None) or (factors is
                               None), 'Either shape or factors must be None'
    assert (shape is not None) or (
        factors is not None), 'Either shape or factors must be not None'
    ...
    aff2[0:3, 3] = aff2[0:3, 3] + aff[0:3, 0:3] * (
        0.5 * np.array([[factors[0]], [factors[1]], [1]]) - 0.5)
