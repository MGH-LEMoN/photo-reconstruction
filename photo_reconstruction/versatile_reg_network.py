...
# def slice_wise_LNCC(I, J, win_semilen=3):
#
#     # set window size
#
#     I_rearranged = torch.unsqueeze(torch.unsqueeze(I, dim=0), dim=0)
#     J_rearranged = torch.unsqueeze(torch.unsqueeze(J, dim=0), dim=0)
#
#     win = [1 + 2 * win_semilen, 1 + 2 * win_semilen, 1]
#     sum_filt = torch.ones([1, 1, *win]).to(I.device)
#     pad_no = win_semilen
#     stride = (1, 1, 1)
#     padding = (pad_no, pad_no, 1)
#
#     # get convolution function
#     conv = getattr(torch.nn.functional, 'conv3d')
#
#     # compute CC squares
#     I2 = I_rearranged * I_rearranged
#     J2 = J_rearranged * J_rearranged
#     IJ = I_rearranged * J_rearranged
#
#     I_sum = conv(I_rearranged, sum_filt, stride=stride, padding=padding)
#     J_sum = conv(J_rearranged, sum_filt, stride=stride, padding=padding)
...
class PhotoAligner(nn.Module):
    ...
    def __init__(self, photo_vol, photo_mask_vol, photo_aff, mri_vol, mri_aff, ref_surf, device='cpu',
                 allow_scaling_and_shear=False, allow_sz=False,
                 k_dice_mri=0.8, k_ncc_intermodality=0.8, k_surface_term=0.8,
                 k_dice_slices=0.1, k_ncc_slices=0.1, k_regularizer=0.01, pad_ignore=None,
                 t_ini=None, theta_ini=None, shear_ini=None, scaling_ini=None, sz_ini=None, ref_type='mask',
                 allow_s_reference=False, t_reference_ini=None, theta_reference_ini=None, s_reference_ini=None,
                 allow_nonlin = False, field_ini=None, k_nonlinear=0.1):
        ...
        self.photo_rearranged = torch.unsqueeze(self.photo_vol.permute(3, 0, 1, 2), dim=0).to(self.device)
        self.photo_mask_vol = torch.Tensor(photo_mask_vol).to(self.device)
        self.mask_rearranged = torch.unsqueeze(torch.unsqueeze(self.photo_mask_vol, dim=0), dim=0).to(self.device)
        ...
        if allow_s_reference:
            if s_reference_ini is not None:
                self.s_reference = torch.nn.Parameter(torch.tensor(s_reference_ini).to(self.device))
            else:
        ...
        if t_reference_ini is not None:
            self.t_reference = torch.nn.Parameter(torch.tensor(t_reference_ini).to(self.device))
        else:
        ...
        if theta_reference_ini is not None:
            self.theta_reference = torch.nn.Parameter(torch.tensor(theta_reference_ini).to(self.device))
        else:
        ...
        if allow_nonlin:
            if field_ini is not None:
                ...
                self.field = torch.nn.Parameter(torch.zeros(5, 5, 2, self.Nslices).to(self.device))
    ...
    def forward(self):
        ...
        theta_reference_f = self.theta_reference / 180 * torch.tensor(np.pi)  # degrees -> radians
        t_reference_f = self.t_reference  # no scaling
        s_reference_f = torch.exp(self.s_reference / 20) # ensures positive and symmetry around 1 in log scale
        ...
        if self.field is not None:
            ...
            field_fullsiz = torch.nn.Upsample(size=self.photo_vol.shape[:-2], align_corners=True, mode='bilinear')(field_pixels)
            ...
            grad_xx = (field_fullsiz[0, 2:, 1:-1, :] - field_fullsiz[0, :-2, 1:-1, :]) / 2.0
            grad_xy = (field_fullsiz[0, 1:-1, 2:, :] - field_fullsiz[0, 1:-1, :-2, :]) / 2.0
            grad_x = torch.sqrt(torch.clamp(grad_xx * grad_xx + grad_xy * grad_xy, min=1e-5))
            grad_yx = (field_fullsiz[1, 2:, 1:-1, :] - field_fullsiz[1, :-2, 1:-1, :]) / 2.0
            grad_yy = (field_fullsiz[1, 1:-1, 2:, :] - field_fullsiz[1, 1:-1, :-2, :]) / 2.0
            grad_y = torch.sqrt(torch.clamp(grad_yx * grad_yx + grad_yy * grad_yy, min=1e-5))
        ...
        for z in range(self.Nslices):
            ...
            if field_fullsiz_rearranged is None:
                for d in range(3):
                    grids_new[d, :, :, z] = T[d, 0, z] * self.grids[0, :, :, z] + T[d, 1, z] * self.grids[1, :, :, z] +  T[d, 2, z] * self.grids[2, :, :, z] + T[d, 3, z]
            else:
                for d in range(2):
                    grids_new[d, :, :, z] = T[d, 0, z] * self.grids[0, :, :, z] + T[d, 1, z] * self.grids[1, :, :, z] + T[d, 2, z] * self.grids[2, :, :, z] + T[d, 3, z] + field_fullsiz_rearranged[d, :, :, z]
                grids_new[2, :, :, z] = T[2, 0, z] * self.grids[0, :, :, z] + T[2, 1, z] * self.grids[1, :, :, z] + T[2, 2, z] * self.grids[2, :, :, z] + T[2, 3, z]
        ...
        for i in range(3):
            grids_new[:, :, :, :, i] = 2 * (grids_new[:, :, :, :, i] / (self.photo_mask_vol.shape[i] - 1) - 0.5)
        ...
        mask_resampled  = nnf.grid_sample(self.mask_rearranged,grids_new, align_corners=True, mode='bilinear', padding_mode='zeros')
        ...
        if self.ref_type == 'surface':
            ...
            xx = combo[0, 0] * self.ref_surf[:, 0] + combo[0, 1] * self.ref_surf[:, 1] + combo[0, 2] * self.ref_surf[:, 2] + combo[0, 3]
            yy = combo[1, 0] * self.ref_surf[:, 0] + combo[1, 1] * self.ref_surf[:, 1] + combo[1, 2] * self.ref_surf[:, 2] + combo[1, 3]
            zz = combo[2, 0] * self.ref_surf[:, 0] + combo[2, 1] * self.ref_surf[:, 1] + combo[2, 2] * self.ref_surf[:, 2] + combo[2, 3]

            ok = (xx > 0)  & (yy > 0)  & (zz > 0)  & (xx < photo_resampled.shape[0]-1)  & (yy < photo_resampled.shape[1]-1)  & (zz < photo_resampled.shape[2]-1)
        ...
        for d in range(3):
            grids_new_mri[d, :, :, :] = D[d, 0] * self.grids[0, :, :, :] + D[d, 1] * self.grids[1, :, :, :] + D[d, 2] * self.grids[2, :, :, :] + grids_new_mri[d, :, :, :] + D[d, 3]

        grids_new_mri = torch.unsqueeze(grids_new_mri, 0)
        grids_new_mri = grids_new_mri.permute(0,2,3,4,1)
        for i in range(3):
            grids_new_mri[:, :, :, :, i] = 2 * (grids_new_mri[:, :, :, :, i] / (self.mri_vol.shape[i] - 1) - 0.5)
         # Not sure why, but channels need to be reversed
        grids_new_mri = grids_new_mri[..., [2, 1, 0]]
        mri_resampled  = nnf.grid_sample(self.mri_rearranged,grids_new_mri, align_corners=True, mode='bilinear', padding_mode='zeros')
        ...
        if self.ref_type == 'image':
            ...
            nccs = torch.zeros(3, self.Nslices - self.pad_ignore[0] - self.pad_ignore[1]).to(self.device)
            for z in range(self.Nslices - self.pad_ignore[0] - self.pad_ignore[1]):
                ...
                for c in range(3):
                    ...
                    my = torch.mean(y)
                    vy = y - my
                    nccs[c, z] = torch.mean(vx * vy) / torch.sqrt(  torch.clamp(torch.mean(vx ** 2), min=1e-5)  ) / torch.sqrt(  torch.clamp(torch.mean(vy ** 2) , min=1e-5) )
        ...
        dices_slices = torch.zeros(self.Nslices - 1 - self.pad_ignore[0] - self.pad_ignore[1]).to(self.device)
        ...
        nccs_slices = torch.zeros(self.Nslices - 1 - self.pad_ignore[0] - self.pad_ignore[1]).to(self.device)
        for z in range(self.Nslices - 1 - self.pad_ignore[0] - self.pad_ignore[1]):
            x = photo_resampled[:, :, z + self.pad_ignore[0], ...]
            y = photo_resampled[:, :, z + self.pad_ignore[0] + 1, ...]
            m = mask_resampled[:, :, z + self.pad_ignore[0]] * mask_resampled[:, :, z + self.pad_ignore[0] + 1]
            ...
            vx = x - mx
            vy = y - my
            nccs_slices[z] = torch.sum(vx * vy * m3) / torch.clamp(torch.sqrt(torch.sum(m3 * (vx ** 2))) * torch.sqrt(torch.sum(m3 * (vy ** 2))), min=1e-5)
        ...
        return loss, photo_resampled, photo_aff, mri_aff_combined, Rt
